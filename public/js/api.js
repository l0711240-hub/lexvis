...
// ── 헌재결정례 ───────────────────────────────────────────────────
export const searchDetc = (query, { page=1, display=20, sort='ddes' } = {}) => {
  const p = new URLSearchParams({ query, page, display, sort });
  return apiFetch(`/api/detc/search?${p}`);
};

export const getDetcDetail = (id) => apiFetch(`/api/detc/detail/${encodeURIComponent(id)}`);
